...
{
	AddToRoot();
	EditorEnv = MakeShared<UnLua::FLuaEditorEnv>();
	EditorEnv->Start();
	for (auto ClassPtr : LuaBindClasses) {
		EditorEnv->RegisterClass(ClassPtr.ToString());
	}
	EditorEnv->TryBind(this);
	Startup();
}
...
void UUnLuaEditor::SetTabWidget(FName TabId, UUserWidget* UserWidget, bool CanClose)
{
	if (UserWidget) {
		TSharedPtr<FTabManager> TabManager = GetTabManager();
		if (TabManager.IsValid()) {
			auto TabInfo = TabWidgetInfos.Find(TabId);
			if (TabInfo) {
				if (TabInfo->TabId == TabId && TabInfo->UserWidget == UserWidget && TabInfo->CanClose == CanClose) {
					return;
				}
				TabWidgetInfos.Remove(TabId);
			}
			bNeedRecreateLayout = true;
			TabManager->UnregisterTabSpawner(TabId);
			TabWidgetInfos.Add(TabId, TabWigdetInfo({ TabId, UserWidget, CanClose }));
